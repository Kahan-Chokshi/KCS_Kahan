...
const ValidationDemo = () => {
    ...
    const onValidate = (e,regEx) => {
        ...
        if(e.target.value==="" || RegExObj.test(e.target.value))
        {
            ...
            // if(e.type==='change')
            // {
            //     setUser({...user, [e.target.name]:e.target.value.slice(0, -1)});
            // }
        }
    }
    ...
    return (
        <FormGroup className={classes.container}>
            ...
            <br/>
            <FormControl>
                <Button variant="contained" color="primary" disabled={name.length===0 || phone.length===0 || email.length===0 || password.length===0 || isemail!==""|| isname!==""|| isphone!==""|| ispassword!==""} onClick={() => validateDetails()}>Add User</Button>
            </FormControl>
        </FormGroup>
    )
}
